...
/** Drives {@link EvaluationStrategyTCK} against {@link AgentEvaluationStrategy}. */
final class AgentEvaluationStrategyTckTest extends EvaluationStrategyTCK {
                ...
                ctx ->
                        Mono.just(
                                new EvaluationVerdict(
                                        ctx.artifact().isBlank()
                                                ? VerdictOutcome.REVISE
                                                : VerdictOutcome.PASS,
                                        ctx.artifact().isBlank() ? 0.0 : 1.0,
                                        "stub-judge",
                                        List.of(),
                                        Instant.now())));
    }
    ...
    protected EvaluationStrategy crashingStrategyUnderTest() {
        return new AgentEvaluationStrategy(ctx -> Mono.error(new RuntimeException("judge boom")));
    }
}
