 ...
 *
 * <p><b>Cancellation safety:</b> Uses {@link Mono#create} with {@code sink.onDispose()} to register
 * a cleanup action that interrupts the reading thread when the Reactor subscription is disposed
 * (e.g., by Ctrl+C). The reading loop uses {@link Reader#ready()} with short sleeps so it responds
 * to {@link Thread#interrupt()} within ~50ms — unlike a raw {@code readLine()} which blocks
 * uninterruptibly on {@code System.in}.
 */
public final class ConsoleApprovalHandler implements UserApprovalHandler {
    ...
    /**
     * Create a handler bound to {@code System.in} / {@code System.out}. Suitable for simple CLI
     * use; for JLine-managed terminals, prefer the explicit-streams constructor.
     */
    ...
    public Mono<ApprovalResult> requestApproval(ToolCallRequest request) {
        ...
        return Mono.<ApprovalResult>create(
                        sink -> {
                            Thread thread =
                                    new Thread(
                                            () -> {
                                                try {
                                                    ...
                                                    sink.success(
                                                            ApprovalResult.denied("Cancelled"));
                                                } catch (Exception e) {
    ...
    /**
     * Prompt the user on the console and return their decision. Synchronized so only one prompt is
     * active at a time.
     *
     ...
     *
     * <p>Instead of blocking on {@code reader.readLine()} (which ignores {@code
     * Thread.interrupt()}), this method polls {@code reader.ready()} and sleeps between checks. The
     * sleep is interruptible, so when the Reactor subscription is disposed and the thread is
     * interrupted, we exit cleanly.
     *
    ...
    /** Clear all remembered approval decisions. */
    public void resetApprovals() {
        approvalMemory.clear();
    }

    /**
     * Return a snapshot of the current approval state. Used by session-resume code paths to persist
    ...
    /**
     * Restore approval state from a previous snapshot.
     *
