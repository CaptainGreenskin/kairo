...
public class MultiAgentDemo {
    ...
    public static void main(String[] args) {
        System.out.println("=== Kairo Multi-Agent Orchestration Demo ===");
        ...
        Task designTask = taskBoard.create("Design API schema",
                "Design the REST API schema for the user service");
        System.out.println("  Created: #" + designTask.id() + " — " + designTask.subject());

        Task implTask = taskBoard.create("Implement endpoints",
                "Implement the REST endpoints based on the API schema");
        System.out.println("  Created: #" + implTask.id() + " — " + implTask.subject());

        Task testTask = taskBoard.create("Write tests",
        ...
        System.out.println("  Unblocked tasks: " + unblocked.stream()
                .map(t -> "#" + t.id() + "(" + t.subject() + ")").toList());
        ...
        System.out.println("  Unblocked now: " + unblocked.stream()
                .map(t -> "#" + t.id() + "(" + t.subject() + ")").toList());
        ...
        messageBus.receive("coder").subscribe(msg ->
                System.out.println("  [coder received] " + msg.text()));
        messageBus.receive("tester").subscribe(msg ->
                System.out.println("  [tester received] " + msg.text()));

        // Send direct message
        System.out.println("  architect → coder: sending design spec...");
        messageBus.send("architect", "coder",
                Msg.of(MsgRole.ASSISTANT, "API schema finalized: GET/POST/PUT/DELETE /users/{id}"))
                .block();

        // Broadcast to all
        System.out.println("\n  coder → broadcast: code ready...");
        messageBus.broadcast("coder",
        ...
        System.out.println("\n========================================");
        System.out.println("  Multi-Agent Demo complete!");
