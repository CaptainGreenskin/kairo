...
Kairo 的团队协调通过 `InProcessMessageBus` 和 `DefaultTeamManager` 实现。核心设计决策是采用 Reactor `Sinks.Many` 做响应式消息推送，同时保留 `ConcurrentLinkedQueue` 作为可靠缓冲。为什么要双模？源于一个实践教训：纯响应式方案在订阅者尚未就绪时会丢消息，纯队列方案无法支持实时推送。双模确保消息既不丢失，又能零延迟送达——队列兜底可靠性，Sink 负责推送时效性。
...
`ExpertTeamStateMachine` 定义了严格的生命周期——九个状态，每个状态转换都经过验证，非法转换触发 `IllegalStateException`。这里的设计哲学是：多 Agent 编排不能有"模糊地带"。结合第六篇讨论的 Hook 治理体系（30 个生命周期点 x 5 种决策），编排的每一步都可以被拦截和治理。`EVALUATING → GENERATING` 这条回边是评审-修订循环：Evaluator 判定产出不合格，状态回到 GENERATING，同一个 Agent 带着评审反馈重新生成。循环有预算限制（`maxFeedbackRounds`），超出后根据风险等级决定是降级通过还是直接失败。
...
Kairo 的 `MixtureOfAgentsCoordinator` 通过 builder 模式组装：添加多个 proposer（不同模型提供商各自独立生成答案）和一个聚合器（融合所有答案）。API 刻意保持极简——每个 proposer 只需指定 ModelProvider、模型名和标签。这反映了 MoA 的核心理念：价值来自模式本身的简洁性，不来自配置的灵活性。
