...
回头看整个系列，十二篇文章围绕一个核心论点：**Agent 的可靠性不来自更强的模型，而来自模型之外的工程基础设施。**

从那个令人清醒的数学事实出发——30 步 Agent，每步 90% 成功率，端到端只有 4.2%——我们逐层拆解了让这个数字变得可接受所需要的基础设施。上下文压缩让 Agent 不会被记忆撑爆；安全与韧性让它在生产环境中不失控；工具设计让它与外部世界有效交互；SPI 让整个系统可扩展、可替换；Hook 让每一步都可以被治理；DurableExecution 让长任务可以恢复；多 Agent 编排让复杂任务可以分解；自进化和 Plugin 让能力可以持续增长。
