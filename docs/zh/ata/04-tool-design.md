...
56 个工具的 schema 占用 11,000 token。一个自然的想法是：能不能只加载相关的工具？
...
说实话，动态工具加载的问题我还没有解决。当前的方案是所有 56 个工具的 schema 始终存在于系统提示中。11,000 token 的固定成本。这是一个已知的效率损失，但比错过一个关键工具的风险更可接受。
...
Agent 的工具调用在结构上很类似。`read`、`edit`、`bash`、`grep`——每个工具有明确的 schema、副作用分类和输出格式。运行时根据这些信息做权限检查、并行调度、预算治理。
...
这个类比不是为了好听。它的实际含义是：工具的设计质量决定了 Agent 运行时能做多少事——能做什么安全检查、能做什么调度优化、能给用户什么级别的控制。系统调用设计得好，操作系统就强。工具设计得好，Agent 运行时就强。
