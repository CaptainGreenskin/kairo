...
每个 Agent 在独立的 worktree 目录中工作，完成后通过 PR 合并——和开发者的多分支工作流完全一样。选择 worktree 而不是自己实现隔离层，是因为 Git 在冲突检测、原子合并、版本回退上已经积累了 20 年的工程。Agent 协调本质上就是分支协调，没必要再发明一套。
...
每个 Agent 获得独立的 worktree 目录、Git 分支、终端会话和上下文窗口——四层隔离，全部由 Git 和终端复用器提供，不需要框架额外实现。
...
`Workspace` 对象只暴露四样东西：ID、根路径、类型标识和元数据。设计上刻意不暴露文件操作方法——工作空间只负责"在哪里"，"怎么操作"由工具和沙箱决定。`WorkspaceKind` 枚举定义了三种工作空间类型：
...
`DurableExecution` 是聚合根，它的核心是一个有序的事件列表。每个 `ExecutionEvent` 携带事件 ID、类型、时间戳、JSON 载荷、哈希值和 schema 版本。其中 `eventHash` 用于构建防篡改的哈希链（下面会展开），`schemaVersion` 用于事件格式的前向兼容——新版本写入的事件不会让旧版本崩溃。

六种事件类型覆盖了 ReAct 循环的全部关键步骤：模型请求/响应、工具请求/响应、上下文压缩、迭代完成。其中 `ITERATION_COMPLETE` 比较特殊——它是一个"安全点"（safe point），标记了一个完整迭代的结束。恢复时，系统从最近的 `ITERATION_COMPLETE` 事件开始重放。

为什么选迭代而不是每次工具调用作为安全点？工具调用之间可能有因果依赖（先删旧接口、再创新接口），从中间恢复会让文件系统处于不一致状态。一个完整的 ReAct 迭代——推理、行动、观察——是最小的"语义完整"单元。
...
**默认策略是保守的**：没有注解的工具被视为 `@NonIdempotent`——宁可返回旧结果，也不冒险重复执行。这个默认值的选择是反复权衡的结果：默认幂等会让不安全的工具在恢复时被重复执行（比如重复发起 HTTP POST），后果不可预测；默认非幂等最多让幂等工具浪费一次机会读取最新数据，但不会造成副作用。安全侧的默认值永远比性能侧的默认值更重要。

### 并发控制：乐观锁

`DurableExecution` 带有一个 `version` 字段，用于乐观锁——`updateStatus` 方法要求传入期望的版本号，版本不匹配则更新失败。这个设计把并发控制分成了两层：执行记录的状态更新用乐观锁，事件日志是 append-only（只 INSERT，不 UPDATE 也不 DELETE）。追加操作不需要锁，高频的事件写入不会成为瓶颈。
...
生产部署使用 JDBC 实现，只有两张表：`kairo_executions`（执行记录，带 version 列做乐观锁）和 `kairo_execution_events`（事件日志，append-only）。没有 ORM，没有 JPA，就是 JDBC + Flyway。选择这种"原始"方案的原因是：持久化执行的存储层必须极度可预测——ORM 的延迟加载和缓存在崩溃恢复场景下会带来额外的不确定性。
...
每个 `ResourceConstraint` 实现检查一个维度的约束。`ResourceContext` 提供了四样检查输入：当前迭代次数、累积 token 消耗、已运行的墙钟时间、Agent 状态快照。有意不提供工具调用历史——约束应该基于资源维度判断，而不是介入 Agent 的具体行为（那是 Hook 的事）。
...
三个内置约束覆盖了最常见的场景：`MaxIterationConstraint` 和 `TokenBudgetConstraint` 触发 `GRACEFUL_EXIT`，`TimeoutConstraint` 触发 `EMERGENCY_STOP`。前两者允许 Agent 完成当前迭代、保存状态、写入 checkpoint；超时直接终止，因为超时通常意味着某些东西卡住了（死锁、网络悬挂），继续等待已经没有意义。

**组合语义**：多个约束按注册顺序依次评估，最严重的 action 胜出。第一个 `EMERGENCY_STOP` 触发短路——后续约束不再评估。所有的 `ResourceValidation` 结果都被收集（即使短路），用于可观测性。用户可以通过注册自定义 `ResourceConstraint` Bean 来添加成本上限、合规限制等维度的约束。

这个 SPI 体现了渐进响应的思路——不是二元的"停或不停"，而是四个级别的分级响应。选择四级而不是两级（停/不停），是因为我们观察到长任务的资源消耗曲线不是线性的——Agent 往往在最后 20% 的迭代中消耗 50% 的 token（反复修复测试、处理边缘情况）。提前警告给了 Agent 加速收敛的信号，而不是在毫无预兆的情况下被截断。
...
**第 0 步：启动。** `DurableExecutionStore.persist()` 创建执行记录，`WorkspaceProvider.acquire()` 分配独立 worktree。Agent 获得隔离的工作空间。
...
**恢复。** `listPending()` 找到未完成的执行，`recover()` 验证哈希链完整性（hash_0 到 hash_95 全部校验通过），从第 95 步的 `ITERATION_COMPLETE` 安全点加载 checkpoint。Agent 恢复到第 95 步的状态。第 96 步对 `AuthControllerTest.java` 的半成品修改还在文件系统上——但 Agent 从 checkpoint 知道它还没有完成这个修改，会重新执行。
...
3. VILA-Lab, "Dive into Claude Code: The Design Space of Today's and Future AI Agent Systems," arXiv:2604.14228, April 2026
