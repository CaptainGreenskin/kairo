...
| init 回收孤儿 | `recoverSessionPhase`（崩溃自愈） | 进程挂了，重启后识别僵尸 Session |
| `execve` | `rebuildSession`（热重建） | 换 Agent（凭证/模型），不换会话 |
| undo log / checkpoint | `rewindToIteration`（按迭代回溯） | 对话 + 文件一起退到第 n 步 |
...
## 崩溃、重建、回溯：把 Session 当进程来管

前面讲的"恢复"解决的是**跨会话**的延续性——关掉浏览器，明天回来。但还有一类问题在**单次会话内部**：服务挂了怎么办、Agent 的配置变了要不要重开会话、Agent 走错路了能不能退回去。

这几个问题合起来，就是 Session 的"韧性"。操作系统为进程提供了一整套对应物——core dump 后重启、`execve` 换代码不换进程、undo log 回滚到检查点。我们逐一造了。

### 崩溃自愈：识别僵尸 Session

最尴尬的故障：服务进程在 Agent 跑到一半时崩了（OOM、断电、`kill -9`）。重启之后，那条 Session 在持久化文件里还记着 `EXECUTING`。但内存里的 Agent 已经没了。用户点进来，UI 显示"运行中"，永远等不到结果——一条**僵尸 Session**。

处理逻辑很直接：建会话时读持久化的 phase，如果还停在 `EXECUTING` 或 `PLANNING`，直接判成 `FAILED_EXECUTION`。宁可让用户看到"上次中断了，请回退后重试"，也不要让他盯着一个永远不会回来的"运行中"。

```java
// 建会话时：上次崩在执行中 → 标记失败，而非假装还在跑
recoverSessionPhase(config.workingDir(), ctx.phaseRef());
```

逻辑很小，但它把"服务崩溃"的用户体验从"莫名其妙地卡死"变成了"明确的失败 + 一条出路"。从 `FAILED_EXECUTION` 有两条出口：`revert`（回退工作区到运行前快照）或 `resume`（重新驱动）。

OS 里这对应 init 回收孤儿进程——内核不会让一个父进程已死的子进程永远挂着。

### 热重建：换引擎，不换车

用户的 API key 过期了，或者想换个模型继续。传统做法是开个新会话、把上下文重新描述一遍。但这破坏了连续性——明明是同一个任务，只是换了凭证，为什么要用户重新开场？

`rebuildSession` 做的是**热重建**：保留同一个 sessionId 和它的对话历史，只把底层的 Agent 对象换掉。新 Agent 用新凭证/新模型重建，conversation history 原样接上。对用户来说，就像进程做了 `execve`——PID 不变，代码段换了。

约束：不能在 Agent 运行时重建（`rebuildAgent` 在运行态会抛 `IllegalStateException`）。这和 `execve` 不能在进程持有锁时进行是一个道理——先停下来，再换。

### 按迭代回溯：对话和文件一起退回去

这是最复杂的一个。用户说："Agent 刚才改的方向不对，回到三步之前。"

LLM 的上下文是线性消息流。"退回三步"意味着两件事都得做：(1) 对话历史回到三步前，(2) Agent 在那三步里改的文件也得撤销。只回退对话不回退文件，Agent 接着说"我已经改好了 X"但文件里没有 X，认知和现实就脱节了。

每跑完一个迭代，`setOnIterationComplete` 会同时落两样东西：

- **对话 checkpoint**：完整消息历史写到 `iter-NNN.json`。
- **工作区快照**：`git stash create -u` 把当时的文件状态（含未跟踪文件）存成一个 stash ref。

回溯 `rewindToIteration(sessionId, n)` 是个六步动作：

1. 加载第 n 次迭代的对话历史。
2. 删掉 n 之后的 checkpoint（保证 `loadLast() == n`，下次恢复从 n 继续，而不是悄悄又跑到旧终点）。
3. 从第 n 次的快照恢复文件（`revertToIteration`，仅 git 工作区，best-effort）。
4. 用 `replaceHistory` 把内存里 Agent 的对话替换成第 n 次的。
5. phase 重置为 `IDLE`——是"可继续"，不是终态。
6. 发 `SESSION_RESTORED` 事件。

```java
// 回溯到一个迭代：对话 + 文件一起退
asp.rewindHistory(history);                              // 内存对话 → 第 n 次
workspaceSnapshotService.revertToIteration(workingDir, n); // 文件 → 第 n 次
phaseRef.set(SessionPhase.IDLE);                         // 可继续，不是终态
```

**一个差点踩的坑：文件恢复绝不能用裸 `git checkout` 单文件。** 用户在跑 Agent 之前可能就有自己未提交的改动。如果回溯时 `git checkout <file> <snapshot>`，会把用户跑 Agent **之前**的本地改动一并冲掉——他根本没想让 Agent 管这部分。正确做法是相对"运行前快照"还原：只回退 Agent 动过的部分，用户的原始改动原样保留。这个区别在 changed-files 面板（逐个 accept/reject Agent 的改动）里反复强调过——快照级还原，不是文件级 checkout。

**退化策略。** 这套依赖 git。非 git 工作区没有快照能力——回溯只回退对话，文件改不了。我们没硬撑着给非 git 也造一套（复制整个工作区代价太大），而是诚实地降级：明确告诉你"文件无法回退，仅对话回退"，剩下的交给你处理。

**保留窗口。** 每个迭代都存快照会爆磁盘。`KAIRO_REWIND_RETENTION` 控制保留最近多少个迭代的快照。回溯只能回到窗口内——超出窗口的迭代 checkpoint 已被清理。这是个工程妥协：无限回溯等于无限存储。

```bash
KAIRO_REWIND_RETENTION=20   # 保留最近 20 个迭代的可回溯快照
```

OS 里这对应数据库的 undo log + 检查点——回滚到一个一致状态。差别是数据库的 page 能 copy-on-write，LLM 的对话历史只能整体替换。

回头看，这三件事——崩溃自愈、热重建、按迭代回溯——本质上是把 OS 进程管理里那套"生死、替换、撤销"搬到了无状态的 LLM 之上。LLM 自己没有进程的概念，但 Session 这一层给它造了一个。
...
### Kairo：压缩摘要 + 隔离 + LRU 池 + 按迭代回溯

跨会话层面不存全量历史，用压缩摘要注入；会话内层面做 per-iteration checkpoint + git 工作区快照，支持**按迭代回溯**（对话和文件一起退，带 retention 窗口），外加崩溃自愈（僵尸 Session 判定）和热重建（换 Agent 不换会话）。

这是个分层妥协。比 Claude Code 多了跨会话延续性和会话内回溯；比 ChatGPT 少了完整历史的全量回看；比 LangGraph 少了真正的多分支分叉（我们只做覆盖式回溯，不做 copy-on-write 分叉）。换来的是资源开销可控——回溯只在 git 工作区 + retention 窗口内，非 git 降级为对话回退。对于对话式 Coding Agent 场景够用，但场景复杂化后（长任务、多分支探索）这套会不会不够，还要看。
...
**Session 分叉。** 用户说"回到五分钟前那个版本，走另一条路"。在 OS 里这是 `fork()`——复制进程的全部状态，两条路径独立走。我们做的是**回溯**——回到第 n 个迭代（对话 + 文件一起退，见前文），但回溯是"覆盖式"的：退回去之后，原来的后续就没了。真正的分叉是"保留两条并行的未来"——从同一个检查点长出两棵独立的对话树。这要求上下文能 copy-on-write，而 LLM 的对话历史是线性消息流，只能整体替换。LangGraph 的多分支 checkpoint 在这里有天然优势。从"回溯"到"分叉"，差的就是这一步 copy-on-write。
