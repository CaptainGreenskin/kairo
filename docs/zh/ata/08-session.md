...
最初所有 Session 共享一个 checkpoint 目录。这是偷懒——"反正每个用户不会同时用嘛"。
...
60 分钟这个数字是拍脑袋定的，说实话我们也不确定是不是最优。太短了用户体验差（去开个会回来 Session 就没了），太长了内存扛不住。实际跑下来 60 分钟在我们的场景没出什么问题，但不同产品形态可能需要完全不同的数字。
