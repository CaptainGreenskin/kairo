...
三家公司各自实现一个 `ModelProvider` 接口，注册到框架中，然后用同一套上下文管理、工具执行、安全护栏和 Hook 生命周期来运行各自的 Agent。运行时的代码，一行都不用改。

SPI 驱动的扩展性，说白了就是：框架不关心你用什么模型、什么存储、什么安全策略。它提供契约，你提供实现。

但抽象有代价。我见过一个团队为了"未来可能的需求"引入了 7 层抽象，半年后每改一行业务代码要改 14 个文件。那个项目最终被重写了。过早的抽象比过早的优化更危险——优化做错了，你得到的是丑陋但正确的代码；抽象做错了，你得到的是优雅但无法修改的架构。

Kairo 自己也交过学费。第一版有 7 个 SPI 只存活到了 v0.3 就被砍掉。
