...
1980 年代，个人计算机的物理内存是 640KB。当时的人觉得够用了。然后程序越来越大，640KB 不够了。操作系统的回答是虚拟内存：把不常用的内存页交换到磁盘，需要时再换回来。LRU（Least Recently Used）、LFU（Least Frequently Used）——一系列页面替换算法诞生。
...
flowchart TD
    subgraph 行业方案
        CC["Claude Code<br/>5层渐进管线<br/>有损压缩"]
        M0["Mem0<br/>独立记忆层<br/>跨会话摘要"]
        LC["LangChain<br/>RAG 检索<br/>语义搜索"]
        MF["Morph FlashCompact<br/>Observation Masking<br/>工具输出优先删"]
        TL["TALE<br/>Token Elasticity Zones<br/>按弹性分级"]
    end
    subgraph Kairo
        K["6阶段压缩引擎<br/>= Claude Code思路<br/>+ 事实优先策略<br/>+ Hook 可插拔"]
    end
    CC -.->|启发| K
    MF -.->|验证| K
    TL -.->|理论支撑| K
...
```mermaid
flowchart LR
    A[Snip<br/>占位符替换<br/>触发: 80%] --> B[Micro<br/>结构感知裁剪<br/>触发: 85%]
    B --> C[Collapse<br/>规则折叠<br/>触发: 90%]
    C --> D[Auto<br/>LLM 摘要<br/>触发: 95%]
    D --> E[Partial<br/>选择性丢弃<br/>触发: 98%]
    E --> F[CircuitBreaker<br/>强制截断<br/>触发: 99%]
...
Kairo 的 `RoutingPolicy` SPI（[ADR-013](/adr/)，ADR 是 Architecture Decision Record，记录每个重要架构决策的"为什么"）实现了成本感知路由。`ModelTierRegistry` 将模型分为层级，`CostAwareRoutingPolicy` 根据任务复杂度、上下文大小和预算约束选择最合适的模型。降级是线性的——如果首选模型的价格超出预算，自动降到下一级。
