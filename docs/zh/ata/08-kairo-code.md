...
框架如果不被真实产品验证，就只是学术练习。

这句话听起来像是老生常谈。不过当你真的把自己写的框架拿来构建一个生产级应用时，你会发现它不是一句鸡汤——它是一面镜子，而且是那种会让你不想照的镜子。

Kairo 从第一天起就不只是一个框架。我同时在构建一个完整的 Coding Agent——kairo-code。它不是 demo，不是 example 项目，而是一个真正能用来写代码的产品。它有 56 个工具、支持 4 家模型厂商、提供 CLI 和桌面两种交互形态、具备多 Agent 协作能力。

为什么要做这件事？不是为了取代 Claude Code。是为了回答一个问题：**Kairo 这个框架，能不能承载一个 Claude Code 量级的应用？**

如果不能，框架就没有存在的价值。如果能，框架才有资格说自己是"Agent 操作系统"。

kairo-code 就是这个拷问。

## 三个阶段的进化

kairo-code 不是一开始就有桌面应用的。它经历了三个清晰的阶段，每个阶段都在拷问框架的不同能力。
...
这就够了。不需要 Web 服务器，不需要数据库，不需要 Docker。一个 JVM 进程，一个 API Key，你就有了一个能读写文件、执行命令、运行测试的 Coding Agent。

CLI 阶段暴露了框架的第一批问题。

最直接的问题是流式输出。Agent 的推理过程可能持续 30 秒。如果用户看不到中间过程——只是盯着一个没有反应的光标——体验是不可接受的。框架的 `Agent` 接口只有 `Mono<Msg> call(Msg)`，返回的是最终结果。我不得不通过 Hook 的 `PostReasoning` 和 `PostToolExecute` 事件来拦截中间输出，手动推送到终端。
...
**Dogfooding 的本质就是这个循环：发现问题 → 在应用侧 workaround → 把问题报回框架 → 框架修复 → 应用侧移除 workaround。**

CLI 阶段的 REPL 循环清晰地映射到 Kairo 的 ReAct 引擎：
...
这正是框架应该做的事——承担 98.4% 的基础设施工作，让应用层只关心交互。
...
**Bridge SPI 就是在这个需求下诞生的。**

这是 kairo-code 催生的第一个新 SPI。Bridge SPI 定义了 5 个操作目录——IDE 请求（打开文件、执行命令、应用编辑）被转换为 Kairo Agent 调用，结果通过 WebSocket 流式回传。`kairo-event-stream-ws` 作为 WebSocket 传输层实现了 Bridge SPI 的默认 impl。
...
桌面应用使用 React + Tailwind + Zustand 构建，布局直接借鉴了 VS Code 的经验：
...
不过桌面应用不只是一个好看的壳。它引入了几个 CLI 没有的交互模式：
...
桌面阶段对框架的拷问最深：它要求框架同时支持命令行和图形界面两种交互范式，同时支持单 Agent 和多 Agent 两种运行模式，同时支持单项目和多项目两种工作空间。每一个"同时"都在考验 SPI 的抽象是否到位。

## 56 个工具，11 个类别

kairo-code 的工具数量从最初的 6 个增长到 56 个。每一个工具都在使用 Kairo 的某个 SPI——工具不只是"功能"，它们是框架能力的具象化。
...
kairo-code 做的事情是**装配**，不是**发明**。它把 Kairo 框架的 SPI 组装成一个可运行的编程 Agent——和任何应用使用框架的方式完全一致。
...
如果 kairo-code 需要修改框架内部才能实现某个功能，那说明框架的 SPI 设计不合理——这时候我修的是框架，不是在应用层打补丁。

## 四个被 kairo-code 逼出来的 SPI

这是 dogfooding 最有价值的产出——不是 kairo-code 本身，而是它催生的框架能力。
...
框架没有这个能力。于是 `AcpAgent` SPI 诞生了——定义了 `initialize`、`session.new`、`session.prompt` 三个 JSON-RPC 方法，让编辑器通过 stdio 驱动 Kairo Agent。这是一个 `@Experimental` 的 SPI，MVP 状态，但它证明了一件事：Kairo 的 Agent 不只能被 HTTP/WebSocket 驱动，也能被进程间通信驱动。
...
四个 SPI 的共同点：**它们都不是设计出来的，而是被需求逼出来的。** 没有 kairo-code 的真实场景驱动，这四个抽象不会存在。框架设计者坐在象牙塔里，不可能预见"编辑器想通过 stdio 驱动 Agent"这种需求。只有当你真的去集成 Zed 时，你才会发现框架缺了什么。
...
一句话总结差异：**Same Models. Governable.**

你可以用同样的基础模型——Claude Sonnet、Claude Opus。但在 kairo-code 中，你拥有治理权。30 个 Hook 生命周期点、5 种决策值、中间件管道、熔断器、循环检测——这些不是"高级功能"，而是你作为运营者控制 Agent 行为的手段。
...
"可治理"的含义就在于此——不是信任模型会做对的事，而是在机制层面确保它做不了错的事。
...
其中 Expert Team Canvas 值得单独说。当 kairo-code 使用多 Agent 模式时——比如 plan-generate-evaluate 三 Agent 协调——Canvas 会实时展示：

- 每个 Agent 的当前状态（推理中 / 等待工具 / 完成）
- Agent 之间的消息传递（谁把什么发给了谁）
- 任务依赖图（哪些任务在等待哪些前置任务）

这不是花哨的功能。调试多 Agent 系统最大的痛苦是"看不见发生了什么"——三个 Agent 在后台并发执行，你只看到最终结果。如果结果不对，你不知道是哪个 Agent 在哪一步犯了错。Canvas 让整个协作过程可见。
...
浅色和深色主题？也做了。审批模式的可视化切换？也做了。设置页面（模型选择、API Key 配置、集成管理）？也做了。

坦白说，Web UI 的工作量超出了最初的预期。一个 Coding Agent 的根本价值在于它的 Agent 能力，不在于 UI 好不好看。问题在于：**如果没有良好的 UI，用户甚至不会给你的 Agent 能力一个机会。** 这是产品工程和框架工程的区别——框架可以只关心 API 是否优雅，产品必须关心用户的第一印象。

## kairo-assistant：另一面的验证

一个框架只被一种应用验证，等于没验证。

kairo-code 证明了 Kairo 能承载 Coding Agent。不过 Coding Agent 有其特殊性——CLI 交互、文件读写、代码生成，这些场景对框架的考验集中在工具执行和上下文压缩上。如果换一个截然不同的领域呢？

kairo-assistant 就是那个"截然不同"。它是基于 Kairo 构建的第二个 dogfooding 产品——一个 SRE 运维 Agent。不写代码，诊断故障。不操作文件，操作告警。不面向开发者，面向值班工程师。
...
但 kairo-assistant 对框架的拷问，和 kairo-code 截然不同。
...
8 个通道意味着 8 种消息格式、8 种认证方式、8 种限流策略。DingTalk 的消息卡片和 Slack 的 Block Kit 是两套迥异的 UI 体系。Telegram 的 Markdown 子集和 Discord 的 Embed 是两种截然不同的富文本方案。如果每个通道都写一套 Agent 逻辑，代码量会爆炸。

这是 Gateway/Channel 架构被逼出来的原因。
...
**Gateway 是 Agent OS 的网络栈——Channel 是网卡驱动，Gateway 是 TCP/IP 协议。**
...
如果 kairo-code 证明了框架能承载 Code Agent，kairo-assistant 证明了同一个框架能承载形态迥异的 Agent——从 CLI 到 8 通道 IM 集成，从编码到运维，SPI 的抽象层没有泄漏。

## 流式执行：模型还在想，工具已经跑

传统的 Agent 执行流程是串行的：等模型吐完全部 token → 解析出工具调用 → 执行工具 → 把结果拼回去。整个过程像流水线上的批处理——前一步不完成，后一步不开始。

结果就是一个荒诞的等待：模型花 3 秒决定调用 `Read` 工具读一个文件，但你需要等它把后续 200 个 token 的推理文本全部生成完毕，才能开始读那个文件。文件读取本身只需要 5 毫秒，但用户感知到的延迟是 3 秒 + 5 毫秒。3 秒在等模型想完，5 毫秒在干活。

Kairo 不这么做。

`StreamingToolDetector` 是中枢。它使用 `Flux.scan` 配合可变的 `DetectorState`，从模型的 SSE 流中**增量检测**完整的 tool_use 块。不等全部 token 到齐，不等响应结束。每到达一个 `TOOL_USE_END` 事件块，对应的工具调用立即被发射到下游执行——此时模型可能还在流式输出第二个、第三个工具调用。
...
**空工具调用 ID**。某些模型 provider 在流式块中不附带 tool call ID。`DetectorState` 退化到"最老的活跃工具"策略——按时间戳排序，假设最先开始的工具最先结束。这不完美，但比崩溃好。
...
用户感知到的差异是巨大的。原来的体验：模型思考 3 秒（无反馈）→ 工具开始执行（终端突然冒出一堆输出）。现在的体验：模型开始思考 → 几百毫秒后第一个工具已经在跑 → 推理文本和工具执行交替出现。Agent 感觉像是在"边想边做"，而不是"想完再做"。

**这是一个用户感知不到机制、但体验差异巨大的优化。**

操作系统里的类比是 DMA（Direct Memory Access）。传统方式是 PIO（Programmed I/O）——CPU 发起 I/O 请求，然后轮询等待设备完成，期间什么也干不了。DMA 是 CPU 发起请求后继续执行指令，设备独立完成数据传输，完成后中断通知 CPU。在 Kairo 里，模型是 CPU，工具是 I/O 设备。`StreamingToolDetector` 就是 DMA 控制器——它让模型（CPU）和工具（设备）并行工作，而不是串行等待。

Agent 操作系统的每一层都在追求同一个目标：**让机器的等待时间趋近于零。** 上下文压缩减少了模型处理无用信息的时间，循环检测减少了模型重复劳动的时间，流式工具执行减少了工具等待模型的时间。它们不是独立的优化——它们是同一个设计哲学的不同投影。

## dogfooding 的代价

### Dogfooding 的价值

这个数字值得记录：**Kairo 框架 80% 的 API 是被 kairo-code 的需求驱动出来的。**
...
如果没有 kairo-code，这些东西不会存在。框架设计者不可能预见所有真实场景——只有把框架放到真实产品中，那些设计盲区才会暴露。

### 代价

只是 dogfooding 的代价也是真实的：**你在同时维护两个产品。**
...
最后一个值得记录的事实：**kairo-code 是用 Kairo 框架构建的，但它也是用 Claude Code 帮忙构建的。**

用 Claude Code 来开发 kairo-code——这件事本身就够讽刺的。用一个 Coding Agent 来构建另一个 Coding Agent。没什么深刻的哲学——纯粹是实用主义。Claude Code 是目前最好用的 Coding Agent 工具，我当然会用它来加速开发。

但这也形成了一个有趣的验证：如果 kairo-code 有一天达到了 Claude Code 的能力水平，那它应该能用来构建下一个版本的自己。这是自举（bootstrapping）——编译器用自己编译自己，Agent 用自己构建自己。
...
**kairo-code-core** 是要害。它不包含任何 Agent 逻辑——Agent 逻辑全在 kairo-core 中。kairo-code-core 做的是装配：用合适的模型提供者、工具集、技能和 Hook 构建一个 `DefaultReActAgent`。把 56 个工具注册到 `DefaultToolRegistry`。从工作空间和用户级目录加载 Markdown 技能。配置 `FileSessionStorageProvider` 实现对话持久化。
...
这种分层和 Kairo 框架自身的分层完全一致：API 层定义契约（kairo-api）、Core 层实现逻辑（kairo-core）、Capabilities 层提供能力（kairo-capabilities）、Transports 层处理 I/O（kairo-transports）。kairo-code 的四个模块是这种分层在应用层面的投影。

## 多模型支持

kairo-code 不绑定任何一家模型厂商。这是和 Claude Code 最显著的差异之一。
...
多模型支持的真正价值不在于"支持多少家"，而在于**自主权**。你的 Agent 不应该被一家厂商的 API 绑架。模型会降价、会升级、会宕机。当某家厂商的 API 不可用时，你需要能够无缝切换到备选模型。这是生产环境的底线需求。

---

最好的框架不是设计出来的，是从真实产品中提炼出来的。

kairo-code 的每一个需求，都在拷问 Kairo 的每一个 SPI："你够用吗？" 不够就改。WorkspaceProvider 是这样来的。Bridge SPI 是这样来的。AcpAgent 是这样来的。LspService 是这样来的。

这种诚实，是框架能活下去的唯一原因。

一个从未被产品验证的框架，就像一个从未被编译器检查的类型系统——它可能在理论上是完美的，但在实践中一文不值。kairo-code 不完美。它的 Web UI 还有 bug 要修。它的多 Agent 协作还需要更多真实场景打磨。它的自进化机制还处于早期阶段。

但它是真实的。它每天在被使用。它的每一个缺陷都是框架进化的下一个方向。
