...
**v0.3.0 已关闭项 ❌：** ToolGroup 动态激活（Skill allowedTools 已覆盖）+ ToolResultPostProcessor（@PostActing Hook 已覆盖）
...
| 改动 | 状态 | 关闭理由 |
|------|------|---------|
| ToolGroup 动态激活 | ❌ 关闭 | Skill 系统的 allowedTools 白名单已覆盖此场景 |
| ToolResultPostProcessor | ❌ 关闭 | @PostActing Hook 已支持 ToolResult 修改、ABORT/SKIP/MODIFY 决策 |
...
| Memory 纠正/强化检测 | `MemoryClassifier` SPI — 正则检测用户纠正（"不对"/"应该是"）和正向强化（"对"/"就这样"），自动设置 confidence 和 category。默认实现中英文双语正则，不用 LLM |
...
> Kairo 的定位：嵌入式 Agent 运行时 + 可选平台扩展。
> 内核保持轻量纯粹，平台能力通过模块化扩展提供。

```
┌─────────────────────────────────────────────────────────────────────┐
│  Layer 4: Platform（平台层）— 可选，面向终端用户触达               │
│  kairo-channel-api / kairo-channel-telegram / kairo-channel-dingtalk│
│  kairo-gateway / kairo-dashboard                                    │
├─────────────────────────────────────────────────────────────────────┤
│  Layer 3: Integration（集成层）— 可选，面向框架集成                │
│  kairo-spring-boot-starter / kairo-examples                         │
├─────────────────────────────────────────────────────────────────────┤
│  Layer 2: Extension（扩展层）— 可选，按需引入                     │
│  kairo-tools / kairo-mcp / kairo-multi-agent / kairo-observability  │
├─────────────────────────────────────────────────────────────────────┤
│  Layer 1: Runtime（运行时内核）— 核心，Agent 引擎                 │
│  kairo-core（ReAct loop, Compaction, Model Provider, Skill, Hook） │
├─────────────────────────────────────────────────────────────────────┤
│  Layer 0: Contract（契约层）— 核心，零实现依赖                    │
│  kairo-api（SPI 接口 + Record 数据类型）/ kairo-bom               │
└─────────────────────────────────────────────────────────────────────┘
```

**分层规则：**
- 上层依赖下层，下层不感知上层
- Layer 0-1 是 Kairo 的核心竞争力，保持精简
- Layer 2 每个模块独立可选，缺失不影响 Layer 1 运行
- Layer 3 是框架绑定，当前只有 Spring Boot
- Layer 4 是平台能力，面向需要完整 Agent 平台的用户

**两种用户画像：**

| 用户类型 | 使用层次 | 典型场景 |
|---------|---------|---------|
| 嵌入式用户 | Layer 0-2 | `import kairo-core` 嵌入现有 Java 应用，自己管 HTTP/部署 |
| 平台用户 | Layer 0-4 | 用 Spring Boot Starter + Channel 模块搭建完整 Agent 服务 |

**Layer 4 规划（v0.5.0+）：**

| 模块 | 说明 | 依赖 |
|------|------|------|
| `kairo-channel-api` | Channel SPI — send/receive/webhook 接口定义 | kairo-api |
| `kairo-channel-telegram` | Telegram Bot 实现 | kairo-channel-api |
| `kairo-channel-dingtalk` | 钉钉机器人实现 | kairo-channel-api |
| `kairo-channel-slack` | Slack App 实现 | kairo-channel-api |
| `kairo-channel-wechat` | 企业微信实现 | kairo-channel-api |
| `kairo-gateway` | SSE Bridge + heartbeat + reconnection + bounded buffer | kairo-core |
| `kairo-dashboard` | Grafana dashboard 模板 + token usage 面板 | kairo-observability |

Channel SPI 设计原则：
- 每个渠道是独立 Maven 模块，对 kairo-core 零侵入
- 和 kairo-mcp 的模式一致：`import` 就有能力，不 `import` 就没有
- Channel 实现只负责消息收发，Agent 逻辑完全在 Layer 1

## 模块演进

```
v0.1.0 (API清理)       v0.2.0 (骨架加固)       v0.3.0 (生态连接)       v0.4.0 (生产级)        v0.5.0 (平台层)
─── Layer 0 ───────────────────────────────────────────────────────────────────────────────────────────────────
kairo-api (精简)       kairo-api (异常+Hook)   kairo-api               kairo-api              kairo-api
kairo-bom              kairo-bom               kairo-bom               kairo-bom              kairo-bom
─── Layer 1 ───────────────────────────────────────────────────────────────────────────────────────────────────
kairo-core             kairo-core              kairo-core              kairo-core             kairo-core
─── Layer 2 ───────────────────────────────────────────────────────────────────────────────────────────────────
kairo-tools            kairo-tools             kairo-tools             kairo-tools            kairo-tools
kairo-multi-agent      kairo-multi-agent(容错) kairo-multi-agent       kairo-multi-agent      kairo-multi-agent
kairo-mcp(@Exp)        kairo-mcp(@Exp)         kairo-mcp(正式)         kairo-mcp              kairo-mcp
                                               + kairo-observability   kairo-observability    kairo-observability
─── Layer 3 ───────────────────────────────────────────────────────────────────────────────────────────────────
kairo-spring-boot-     kairo-spring-boot-      kairo-spring-boot-      kairo-spring-boot-     kairo-spring-boot-
  starter                starter                 starter                 starter                starter
kairo-examples         kairo-examples          kairo-examples          kairo-examples         kairo-examples
─── Layer 4 ───────────────────────────────────────────────────────────────────────────────────────────────────
                                                                                              + kairo-channel-api
                                                                                              + kairo-gateway
                                                                                              + kairo-dashboard
...
| 8 | ToolGroup 动态激活 | ❌ 关闭 | Skill allowedTools 已覆盖 | — |
| 9 | ToolResultPostProcessor | ❌ 关闭 | @PostActing Hook 已覆盖 | — |
...
| 20 | Memory 纠正/强化检测 | ✅ 做 | MemoryClassifier SPI，正则检测纠正/强化，自动设置 confidence | v0.4.0 |

v0.2.1 的 2 个点（#1 cancellation + #2 loop detection）待完成。
v0.3.0 SPI + 健壮性（#3, #4, #5, #6, #7, #15）已完成 ✅。工具增强（#8, #9）已关闭（和现有能力重叠）。
...
| IM Channel（Telegram/Slack/飞书/微信） | → v0.5.0 Layer 4 平台层（kairo-channel-*） |
...
1. **分层架构** — Layer 0-1 内核精简，Layer 2-4 按需扩展，上层依赖下层，下层不感知上层
2. **嵌入式优先** — 核心是库（`import` 就能用），平台能力通过可选模块提供
3. **SPI 可扩展** — 每个核心能力都有接口，用户可以替换任何实现
4. **响应式非阻塞** — 所有 I/O 操作基于 Reactor
5. **渐进式采用** — 从 Hello World 到多 Agent 编排到完整平台，复杂度按需引入
6. **不预设策略** — 框架提供能力（Hook、Tracer、MemoryStore），不预设使用方式
7. **扩展模块完全可选** — MCP、OTel、Channel 等扩展模块缺失或连接失败不影响核心功能
...
| v0.3.0 | 健壮性 + SPI | PermissionDecision + tag 过滤 + confidence + Model CircuitBreaker + Dangling Recovery ✅ / ToolGroup + ToolResultPostProcessor ❌ 关闭 | ✅ 已完成 |
...
| v0.5.0 | 平台层 | Channel SPI + Telegram/钉钉/Slack 渠道 + SSE Gateway + Dashboard 模板（Layer 4） | 根据用户反馈 |
