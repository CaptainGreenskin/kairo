...
class ExecutionToolsIntegrationIT {
    ...
    private ToolResult execBash(Map<String, Object> args) {
        return bashTool.stream(args, CTX)
    ...
    void bashTool_simpleCommand_returnsOutput() {
        ToolResult result = execBash(Map.of("command", "echo hello"));
    ...
    void bashTool_commandWithArgs_returnsOutput() {
        ToolResult result = execBash(Map.of("command", "printf '%s %s' foo bar"));
    ...
    void bashTool_commandFails_returnsError() {
        ToolResult result = execBash(Map.of("command", "exit 42"));
    ...
    void bashTool_commandTimeout_returnsError() {
        ...
        ToolResult result = execBash(Map.of("command", "sleep 30", "timeout", 1));
    ...
    void bashTool_longOutput_handledCorrectly() {
        ...
        ToolResult result = execBash(Map.of("command", "seq 1 5000"));
    ...
    void bashTool_stderrCaptured() {
        ...
        ToolResult result = execBash(Map.of("command", "echo error_msg >&2"));
    ...
    void bashTool_workingDirectory_respected(@TempDir Path tempDir) {
        ToolResult result =
                execBash(Map.of("command", "pwd", "workingDirectory", tempDir.toString()));
    ...
    void bashTool_environmentVariables_inherited() {
        ...
        ToolResult result = execBash(Map.of("command", "echo $HOME"));
    ...
    void monitorTool_missingTarget_returnsError() {
        ToolResult result = monitorTool.execute(Map.of(), CTX).block();
    ...
    void monitorTool_nonNumericTarget_returnsError() {
        ToolResult result = monitorTool.execute(Map.of("target", "rm -rf /"), CTX).block();
    ...
    void monitorTool_validPid_returnsOutput() {
        ...
        ToolResult result = monitorTool.execute(Map.of("target", pid), CTX).block();
    ...
    void monitorTool_nonExistentPid_handledGracefully() {
        // Use an unlikely PID
        ToolResult result = monitorTool.execute(Map.of("target", "9999999"), CTX).block();
