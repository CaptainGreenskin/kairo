...
class TeamCreateToolTest {
    ...
    @Test
    void validNameCallsCreate() {
        tool.execute(Map.of("name", "alpha-team"));
        assertEquals(1, teamManager.createdNames.size());
        assertEquals("alpha-team", teamManager.createdNames.get(0));
    }

    @Test
    void successResultIsNotError() {
        ToolResult result = tool.execute(Map.of("name", "beta-team"));
        assertFalse(result.isError());
    }

    @Test
    void successMetadataContainsTeamName() {
        ToolResult result = tool.execute(Map.of("name", "gamma-team"));
        assertEquals("gamma-team", result.metadata().get("teamName"));
    }

    @Test
    void successContentMentionsTeamName() {
        ToolResult result = tool.execute(Map.of("name", "delta-team"));
        assertTrue(result.content().contains("delta-team"));
    }
