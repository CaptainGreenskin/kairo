...
class TeamCreateToolTest {
    ...
    void missingNameIsError() {
        TeamCreateTool tool = new TeamCreateTool(mock(TeamManager.class));
    ...
    @Test
    void validNameCallsTeamManagerCreate() {
        TeamManager manager = mock(TeamManager.class);
        TeamCreateTool tool = new TeamCreateTool(manager);
        tool.execute(Map.of("name", "alpha"));
        verify(manager).create("alpha");
    }
    ...
    void validNameContentMentionsTeamName() {
        ...
        assertThat(result.content()).contains("my-team");
    }
}
