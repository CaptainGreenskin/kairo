...
# 以下三个 cohort 既是 reactor aggregator，也是物理目录 —— leaf 模块就在它们
# 各自的子目录里。每个 leaf 仍然 inherit kairo-parent（通过两级 relativePath
# `<relativePath>../../pom.xml</relativePath>`），aggregator 仅用于群构建：
#   mvn -pl kairo-capabilities -am test  → 跑该 cohort 全部模块。
# Maven 坐标（artifactId）不带 cohort 前缀，依赖者按 artifactId 引用即可。

kairo-capabilities/       ← cohort: 能力模块（业务功能）
  ├─ kairo-tools/         ← 17+ 内置工具
  ├─ kairo-mcp/           ← MCP 协议集成
...
kairo-transports/         ← cohort: I/O 边界（network / IM adapter / event bus）
  ├─ kairo-event-stream/      ← 事件总线 base (transport-agnostic, 零 Spring 依赖)
  ├─ kairo-event-stream-sse/  ← SSE transport (WebFlux)
  ├─ kairo-event-stream-ws/   ← WebSocket transport (WebFlux) + v1.1 Bridge SPI 默认 impl
  └─ kairo-channel-dingtalk/  ← 钉钉 Channel adapter (impl io.kairo.api.gateway.Channel)
  # 未来 kairo-channel-{feishu,slack,telegram} 落到这里形成 cohort

kairo-starters/           ← cohort: Spring Boot AutoConfiguration（每 capability 一对）
  └─ kairo-spring-boot-starter-* × 13 (idiomatic Spring Boot starter convention，
       不要为了"省 module 数"合并进 capability —— 用户 dep 坐标是公开契约)
