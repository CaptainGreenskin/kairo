...
# 以下三个是 reactor aggregator，物理上扁平 sibling、逻辑上分组。
# 模块仍直接 inherit kairo-parent，aggregator 仅用于群构建：
#   mvn -pl kairo-capabilities -am test  → 跑该 cohort 全部模块。
# 不要把这里的"缩进"当目录层级 —— 文件系统里它们都是 ../foo 兄弟。
...
kairo-starters/           ← cohort: Spring Boot AutoConfiguration（每 capability 一对）
  └─ ... 12 个 kairo-spring-boot-starter-* (idiomatic Spring Boot starter convention，
